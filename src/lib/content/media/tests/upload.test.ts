...
describe('uploadMedia', () => {
	...
	it('should upload a valid image file successfully', async () => {
		...
		mockGetPublicUrl.mockReturnValue({
			data: { publicUrl: 'https://example.com/storage/media/123e4567-e89b-12d3-a456-426614174000/some-uuid.jpg' }
		});
		...
		expect(result.url).toBe('https://example.com/storage/media/123e4567-e89b-12d3-a456-426614174000/some-uuid.jpg');
	...
	it('should throw error for missing file', async () => {
		...
		await expect(uploadMedia(null as any, domainId, mockSupabase))
	...
	it('should throw error for invalid domain ID', async () => {
		...
		await expect(uploadMedia(mockFile, 'invalid-uuid', mockSupabase))
			.rejects.toThrow('Valid domain ID (UUID) is required');
	});

	it('should throw error for missing domain ID', async () => {
		const mockFile = createMockFile('test.jpg', 'image/jpeg', 1024);
		
		await expect(uploadMedia(mockFile, '', mockSupabase))
			.rejects.toThrow('Valid domain ID (UUID) is required');
	});
	...
	it('should throw error for file too large', async () => {
		...
		await expect(uploadMedia(largeFile, domainId, mockSupabase))
			.rejects.toThrow('File size exceeds maximum allowed size of 50MB');
	});
	...
	it('should throw error for unsupported file type', async () => {
		...
		await expect(uploadMedia(mockFile, domainId, mockSupabase))
	...
	it('should handle upload error from Supabase', async () => {
		...
		await expect(uploadMedia(mockFile, domainId, mockSupabase))
			.rejects.toThrow('Failed to upload media: Storage error');
	});
	...
	it('should generate correct file extensions for different MIME types', async () => {
		...
		for (const testCase of testCases) {
			...
			expect(result.handle).toMatch(new RegExp(`^${domainId}/[0-9a-f-]{36}\\.${testCase.expectedExt}$`));
		}
	});
});